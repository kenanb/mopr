            ...
            (mopr-sgt::data
             (list
              (mopr-sgt:make-prop-entry
               :info (mopr-reg:get-prop-info-for-schema :isa :Mesh :extent)
               :data (list extent-data))
              (mopr-sgt:make-prop-entry
               :info (mopr-reg:get-prop-info-for-schema :isa :Mesh :points)
...
(defun prim-fn-grid-fv-counts (w h)
  (mopr-sgt:make-prop-entry
   :info (mopr-reg:get-prop-info-for-schema :isa :Mesh :faceVertexCounts)
...
(defun prim-fn-grid-fv-indices (w h dir
  ...
  (let ((contents
    ...
    (mopr-sgt:make-prop-entry
     :info (mopr-reg:get-prop-info-for-schema :isa :Mesh :faceVertexIndices)
     :data (list contents))))
